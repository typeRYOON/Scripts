...
int addEntry() { return system(R"(np "E:\cmder\templates\YankMap.txt")"); }
int listArgs()
{
    std::ifstream file(R"(E:\cmder\templates\YankList.txt)");
    if (!file.is_open()) {
...
{
    path_map paths;
    std::ifstream file(R"(E:\cmder\templates\YankMap.txt)");
    if (!file.is_open()) {
